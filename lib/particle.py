...
def particlefilter(ID=[],charged=False,pTmin=None,pTmax=None,etamin=None,etamax=None):
    """
    ...
    After creating a particlefilter, it is applied to Particle objects.  All
    criteria must be met for the filter to return True, e.g.:

    >>> pf = particlefilter(ID=[211],pTmin=0.5,etamax=2.5)
    ...
    As its name suggests, a particlefilter() is ideal for use with Python's
    builtin filter(), e.g.:

    >>> list(filter(pf,(p1,p2)))
    [True, False]

    In general, particles generated by from_files() may be filtered via

    >>> filter(particlefilter(...), from_files(...))

    Since from_files() yields None to separate events, particlefilter()
    returns True when applied to None.
